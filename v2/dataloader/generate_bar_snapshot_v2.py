...
def _prepare_sz_bar_actions(
    events: pl.DataFrame,
    bars: list[dt.time],
) -> tuple[pl.DataFrame, pl.DataFrame]:
    """Compress SZ fixed-price events without changing type-85 replay."""
    ord_type = pl.col("OrdType").cast(pl.String, strict=False).fill_null("")
    type85_adds = events.filter(
    ...
    # reductions also have zero available quantity and can be discarded.
    fixed_events = (
        dynamic_events.join(replay_keys, on=ORDER_KEY, how="anti")
        .filter(
            ~ord_type.is_in(["49", "85"])
            & pl.col("Price").is_not_null()
            & (pl.col("Price") > 0)
        )
    )
    ...
    bar_checkpoints = (
        pl.DataFrame({"SecurityID": dynamic_ids})
        .join(pl.DataFrame({"Checkpoint": bars}), how="cross")
    )
    event_checkpoints = replay_events.select(
        "SecurityID",
        pl.col("EventTime").alias("Checkpoint"),
    ).unique()
    checkpoints = pl.concat(
        [bar_checkpoints, event_checkpoints],
        how="vertical_relaxed",
    ).unique().sort(["SecurityID", "Checkpoint"])

    compressed = (
        fixed_events.sort(["SecurityID", "EventTime"])
        .join_asof(
            checkpoints,
            left_on="EventTime",
            right_on="Checkpoint",
        ...
        .filter(pl.col("Checkpoint").is_not_null())
        .group_by(["Checkpoint", "SecurityID", "Side", "Price"])
        .agg(pl.col("QtyDelta").sum())
        .filter(pl.col("QtyDelta").abs() > 1e-12)
        .select(
            pl.col("Checkpoint").alias("EventTime"),
            pl.lit(None).alias("SortNo"),
            pl.lit(3).cast(pl.Int8).alias("EventType"),
            pl.lit(None).alias("ChannelNo"),
            "SecurityID", "Side",
            pl.lit(None).alias("ApplSeqNum"),
            "Price",
            pl.lit(None).alias("OrdType"),
            pl.lit(None).alias("OrderStatus"),
            "QtyDelta",
        )
    )
    actions = pl.concat(
        [static_actions, compressed],
        how="vertical_relaxed",
...
def generate_sh_snapshots(*args, **kwargs) -> pl.DataFrame:
    """SH adapter: status events and aggregate closing-auction matching."""
    kwargs["exchange"] = "sh"
    kwargs.setdefault("aggregate_closing_auction", True)
    return generate_bar_snapshots(*args, **kwargs)


def generate_sz_snapshots(*args, **kwargs) -> pl.DataFrame:
    """SZ adapter: order types with compressed type-85 replay."""
    kwargs["exchange"] = "sz"
    kwargs["aggregate_closing_auction"] = False
    return generate_bar_snapshots(*args, **kwargs)

def _read_proc_frame(path: Path, columns: list[str]) -> pl.DataFrame:
    """Read only replay columns; projection is pushed into Parquet scan."""
    return pl.scan_parquet(path).select(columns).collect(engine="streaming")
...
def generate_from_proc(
    ...
    order_columns = (
        ...
        + (["OrdType"] if exchange == "sz" else ["OrderStatus"])
    )
    reduction_columns = [
    ...
    status_events = (
        _read_proc_frame(
            status_path,
            [
                "ChannelNo", "SecurityID", "ApplSeqNum",
                "TransactTime", "TradingPhaseCode",
            ],
        )
    ...
    return generate_bar_snapshots(
        _read_proc_frame(folder / f"{exchange}wt.pq", order_columns),
        _read_proc_frame(folder / f"{exchange}cj.pq", reduction_columns),
        _read_proc_frame(folder / f"{exchange}cancel.pq", reduction_columns),
