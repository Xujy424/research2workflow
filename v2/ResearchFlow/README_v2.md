...
| 0 | `family_transform.py` | 类内 raw/orthogonal/PCA/PLS 统一变换入口，返回统一 `FamilyTransformResult` |
| 1 | `family.py` | 按 family 分组，类内去重，选择代表因子，类内变换，复检解释力，生成类内复合因子 |
...
| `risk.py` | `FactorRiskModel` Barra 风格因子收益、因子协方差、特异风险和股票协方差 |
...
5. `FactorComb/family.py` 直接使用已入库因子，在每个 family 内做去重、代表因子选择、raw/orthogonal/PCA/PLS 类内变换、解释力复检和类内复合。
6. 选择组合分支：
   - `PortfolioRoute.SLEEVE`：走 `FactorComb/sleeve.py`，得到 sleeve 合成信号。
   - `PortfolioRoute.UNIFIED_ALPHA`：走 `FactorComb/alpha.py`，得到统一 alpha。
7. `Portfolio/portfolio.py` 把信号转成股票目标权重。
8. 默认输出：
...
- 去极值、标准化、中性化：`FactorTest/preprocess.py`。因子进入 registry 后不在 `FactorComb` 重复处理。
...
- `FactorComb/family_transform.py` 已统一承载 raw/orthogonal/PCA/PLS 类内变换，主流程通过 `family.transform_method` 和 `family.transform_fit` 配置切换。
...
- `Portfolio/risk.py` 只保留 Barra 风格 `FactorRiskModel`，优化路径统一使用风险因子暴露估计股票协方差。
...
研究员做方法对比时，不需要改流程代码，只改 `ResearchFlowV2Config` 里的子配置。因子值预处理在 `FactorTest` 入库前完成，组合层不再二次 winsorize/zscore/neutralize。
...
cfg = ResearchFlowV2Config(
    route=PortfolioRoute.UNIFIED_ALPHA,
    family=FamilyConfig(clustering_method="hierarchical", transform_method="orthogonal", composite_method="icir"),
