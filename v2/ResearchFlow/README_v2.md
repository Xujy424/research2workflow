...
| 文件 | 职责 |
| --- | --- |
| `portfolio.py` | 把 alpha/sleeve 信号投影成最终股票权重，处理单票上限、换手、ADV、行业约束 |
| `risk.py` | 轻量风险估计、协方差 shrinkage、PSD 修复 |
| `cost.py` | 线性交易成本和冲击成本估计 |
| `regime.py` | 根据外部状态概率对权重做 bounded tilt |

后续如果要接完整 cvxpy 优化器、Barra 风险模型或交易成本模型，也应该放在 `Portfolio` 下，而不是放回 `FactorComb`。
...
- `FactorComb/alpha.py` 是透明的 equal/ICIR 类间 alpha，不是完整 walk-forward ridge、Fama-MacBeth 或 sklearn 模型。
- `Portfolio/portfolio.py` 是快速权重投影，不是完整 cvxpy 优化器。
- `Portfolio/risk.py` 是协方差 shrinkage，不是完整 Barra 风格因子收益率风险模型。
