...
class HF001(FactorBase):
    def __init__(self):
        name = __class__.__name__
        super().__init__(name, root=None)

    def calc(self, date):
        order, deal, cancel, shot = get_tables(date)
        order = calc_cluster(order, shot, 1.2, 5)
        order = calc_aggress(order, shot)
...
if __name__ == '__main__':
    ...
    hf001 = HF001()
    dates = [np.datetime_as_string(date, unit="D").replace("-", "") for date in hf001.dates]
    start_dt, end_dt = bisect.bisect_left(dates, '20250101'), bisect.bisect_right(dates, '20250630')
    calc_dates = dates[start_dt:end_dt]

    hf001.add('20260624')
    print(hf001[calc_dates])
